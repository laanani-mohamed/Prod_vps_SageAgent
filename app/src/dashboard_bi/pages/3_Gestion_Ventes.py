...
with st.expander("🔍 Filtres de recherche", expanded=True):
    ...
    def fetch_filter_options_ventes(schema):
        if not schema:
            return ["Tout"], ["Tout"], ["Tout"]
        try:
            from services.documents_service import get_documents_entete
            import pandas as pd
            docs = get_documents_entete(schema, domaine=[0])
            if not docs:
                return ["Tout"], ["Tout"], ["Tout"]
    ...
    pieces_opts, tiers_opts, intitule_opts = fetch_filter_options_ventes(client_schema)

    col1, col2, col3 = st.columns(3)
    with col1:
        search_piece = st.selectbox("Recherche N° Pièce", options=pieces_opts, help="Numéro exact du document", key="piece_ventes")
    with col2:
        search_tiers = st.selectbox("Recherche Code Client", options=tiers_opts, help="Filtre sur le code client", key="tiers_ventes")
    with col3:
        search_intitule = st.selectbox("Recherche Intitulé Client", options=intitule_opts, help="Filtre sur le nom du client", key="intitule_ventes")

    col4, col5, col6 = st.columns([1, 1, 2])
    with col4:
        date_from = st.date_input("Date début", value=None, key="df_ventes")
    with col5:
        date_to = st.date_input("Date fin", value=None, key="dt_ventes")
    with col6:
        st.write("") # Espacement pour aligner verticalement
        st.write("")
        unpaid_only = st.checkbox("Non réglés uniquement (Reste > 0)", key="unpaid_ventes")
    ...
    st.button(
        ...
        key="btn_ventes",
        disabled=st.session_state.get("loading_ventes", False)
    )

# ---------------------------------------------------------------------------
# Définition des Onglets par type de document
# ---------------------------------------------------------------------------
tab_general, tab_devis, tab_cmd, tab_prep, tab_bl, tab_retour, tab_avoir, tab_fact, tab_fact_c, tab_arch = st.tabs([
    "Général",
    "Devis", 
    "Bon de commande", 
...
tab_configs = [
    (tab_general, [0, 1, 2, 3, 4, 5, 6, 7, 8], "general"),
    (tab_devis, [0], "devis"),
    (tab_cmd, [1], "bon_commande"),
    (tab_prep, [2], "prep_livraison"),
    (tab_bl, [3], "bon_livraison"),
    (tab_retour, [4], "bon_retour"),
    (tab_avoir, [5], "bon_avoir"),
    (tab_fact, [6], "facture"),
    (tab_fact_c, [7], "facture_comptab"),
    (tab_arch, [8], "archive"),
]
    ...
    with tab:
        try:
            with st.spinner("Chargement des documents..."):
                st.session_state["loading_ventes"] = True
            ...
            st.session_state["loading_ventes"] = False
        ...
        if not df_docs.empty:
            ...
            if sel_rows:
                ...
                client_name = df_docs.iloc[idx]["CLIENT INTITULE"]
                ...
                if lignes:
                    ...
                    show_df(df_l_show, key_suffix=f"lignes_{key_prefix}")
                else:
