"""
bi/referentiel/use_cases/doc_ligne_uc.py
"""
from api.referentiel.schemas import DocLigneRequest, ReferentielResponse
...
def execute(req: DocLigneRequest) -> ReferentielResponse:
    repo = get_repo("doc_ligne", req.source_type)
    ...
    if req.with_entete and req.source_type == "archive":
        raw_data = enrichments.add_reste_a_payer(raw_data)
    ...
    return ReferentielResponse(
        ...
        source=source,
        message=None if data else "ligne de document introuvable",
        total_rows=len(data),
        data=data,
    )
