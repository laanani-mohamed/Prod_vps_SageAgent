"""
bi/referentiel/use_cases/article_detail_uc.py
"""
from api.referentiel.schemas import ArticleDetailRequest, ReferentielResponse
from api.referentiel.repositories.factory_repo import get_repo
from api.referentiel.business_logic import serializers

def execute(req: ArticleDetailRequest) -> ReferentielResponse:
    repo = get_repo("article_detail", req.source_type)
    ...
    return ReferentielResponse(
        ...
        source=source,
        message=None if data else "article introuvable",
        total_rows=len(data),
        data=data,
    )
